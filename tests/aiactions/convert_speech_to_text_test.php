...
final class convert_speech_to_text_test extends \advanced_testcase {
    ...
    public function test_constructor(): void {
        ...
        $action = new convert_speech_to_text(
            ...
            audiofile: $file,
            language: 'en-US',
            timestamps: stt_timestamps_type::NONE,
            format: stt_output_format::JSON,
        );
        ...
        $this->assertEquals(stt_timestamps_type::NONE, $action->get_configuration('timestamps'));
        $this->assertEquals(stt_output_format::JSON, $action->get_configuration('format'));
    }
    ...
    public function test_store(): void {
        ...
        $action = new convert_speech_to_text(
            ...
            audiofile: $file,
            language: 'en-US',
            timestamps: stt_timestamps_type::SEGMENT,
            diarization: true,
            format: stt_output_format::SRT,
        );
        ...
        $this->assertEquals(stt_timestamps_type::SEGMENT->value, $record->timestamps);
        $this->assertEquals(stt_output_format::SRT->value, $record->format);
        $this->assertEquals(
            'Agent: Thanks for calling OpenAI support.\nCustomer: Hi, I need help with diarization.',
            $record->text
        );
        $this->assertEquals('english', $record->detectedlanguage);
        $this->assertEquals(1, $record->diarisation);
        $this->assertNotEmpty($record->textsegments);
        $this->assertNotEmpty($record->confidencestats);
    }
